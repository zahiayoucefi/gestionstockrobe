...
export function generateReceiptPDF(data: ReceiptData): void {
  ...
  // Format date correctly for receipt
  const formattedDate = data.date.toLocaleDateString('fr-FR', {
    year: 'numeric',
    month: '2-digit',
    day: '2-digit'
  });
  const formattedTime = data.date.toLocaleTimeString('fr-FR', {
    hour: '2-digit',
    minute: '2-digit'
  });
  
  doc.text(`Date: ${formattedDate} ${formattedTime}`, pageWidth - margin, yPosition, { align: 'right' });
  yPosition += 10;
  ...
  if (hasRentals) {
    ...
    data.items.forEach((item) => {
      if (item.isRental && item.rentalDays) {
        ...
        doc.text(`${item.name}: À retourner le ${formattedReturnDate}`, margin, yPosition);
        yPosition += 6;
      }
    });
    yPosition += 10;
  ...
  // Save
  const fileName = `Recu_${data.customerName.replace(/\s+/g, '_')}_${formattedDate.replace(/\//g, '-')}.pdf`;
  doc.save(fileName);
}